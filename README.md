...
**전화번호 조회 기반 로그인, 매장 및 상품 길찾기 안내, QR 결제, 실시간 영수증 생성 등 다양한 기능을 통합한 스마트 마트 안드로이드 애플리케이션입니다.**
...
| 🧭 길찾기 안내 | A* 알고리즘을 통해 현재 위치 → 목표 위치까지 경로 탐색해 매장 맵에 표시 | | 
...
## 6. 참고 및 주의사항
### 1) 얼굴 인식 시스템과 연동 시 반드시 Firebase 사용자 정보와 pickle DB를 동기화할 것
### 2) QR 영수증은 JSON 형식 기반이며, 모바일 앱 외부에서 직접 디코딩도 가능
### 3) 실내 위치 측정은 BLE 환경에서 정확도가 달라질 수 있음
